...
// СОХРАНЕНИЕ ПОЛЕЙ И ФОРМ
	...
	if (isset($P["savebutton"])) {
		...
		$res=DB("INSERT INTO `_tags` (`id`, `stat`) VALUE ".trim($query,",")." ON DUPLICATE KEY UPDATE `stat`=values(`stat`)");
		
		$query=""; foreach ($P["Int"] as $key=>$val) { $query.="('".$key."','".DBcut($val)."'),"; }
		$res=DB("INSERT INTO `_tags` (`id`, `name`) VALUE ".trim($query,",")." ON DUPLICATE KEY UPDATE `name`=values(`name`)");
		$_SESSION["Msg"]="<div class='C20'></div><div class='SuccessDiv'>Данные успешно сохранены!</div>";
		@header("location: ".$_SERVER["REQUEST_URI"]); exit();
	}

	if (isset($P["addbutton"])) {
		$res=DB("INSERT INTO `_tags` (`name`,`stat`) VALUES ('".DBcut($P["Int0"])."', '".DBcut($P["Inp0"])."')");
...
// ФОРМА ДОБАВЛЕНИЯ
	$AdminText='<h2 style="float:left;">Список тэгов публикаций</h2>';
...
// ВЫВОД ПОЛЕЙ И ФОРМ	
	...
	$data=DB("SELECT `id`,`stat`,`name` FROM `_tags` ORDER BY `name` ASC");
...
// ПРАВАЯ КОЛОНКА	
	$AdminRight=ATextReplace('Tags-Module')."<div class='C30'></div><div class='LinkR'><a href='?cat=adm_settings'>Основные настройки сайта</a></div>";
}
