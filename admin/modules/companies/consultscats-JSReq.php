...
if ($_SESSION['userrole']>2) {
	...
	function HtmlChild_($lvl, $idi) {
		global $items, $count, $id, $stotal, $prev, $table, $link, $VARS; $pid=$items[$idi]["pid"]; $count++; if ($items[$idi]["stat"]==1) { $chk="checked"; }
		...
		if($lvl == 0) $text.='<td class="Act"></td>';
		else $text.='<td class="Act"><a href="?cat='.$link.'_qa&id='.$idi.'" title="Вопросы и ответы">'.AIco('49').'</a></td>';
	...
	GetChild_(0); $text.="<table>".$itext."</table>";
