...
if($GLOBAL["sitekey"] == 1 && $GLOBAL["database"] == 1) {
    ...
    if($data["total"] != 1) {
        ...
        if(isset( $P["savebutton"] )) {
            foreach($P["name"] as $k => $v) {
                ...
                DB( "UPDATE `" . $table2 . "` SET `name`='" . $P["name"][ $k ] . "', `text`='" . $P["text"][ $k ] . "' WHERE (`id`=$k)" );
            }
        ...
        $table_exists = DB("SHOW TABLES WHERE Tables_in_$DataBaseName = '_widget_questions'");
        if(0 == $table_exists['total']) {
            DB("CREATE TABLE `_widget_questions` (`id` mediumint(9) NOT NULL,`link` varchar(255) NOT NULL,`pid` mediumint(9) NOT NULL,`rate` mediumint(9) NOT NULL,`name` text NOT NULL,`text` text NOT NULL) ENGINE=InnoDB DEFAULT CHARSET=utf8;");
            DB("ALTER TABLE `_widget_questions` ADD PRIMARY KEY (`id`), ADD KEY `link` (`link`,`pid`);");
            DB("ALTER TABLE `_widget_questions` MODIFY `id` mediumint(9) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=0;");
        }
        ...
        if($data["total"] != 1) {
            ...
            $AdminText = '<h2>Ответы на вопросы: &laquo;' . $node["name"] . '&raquo;</h2>' . $_SESSION["Msg"];
            $AdminText .= '<form action="' . $_SERVER["REQUEST_URI"] . '" enctype="multipart/form-data" method="post">';

            ### Основные данные
            $AdminText .= $C10 . '<div class="LinkG"><a href="javascript:void(0);" onclick="AddField(\'' . $alias . '\', \'' . $id . '\')">Добавить</a></div>' . $C10 . $C10;
            $AdminText .= '<div class="RoundText" id="Questions">';
            for($i = 0; $i < $data["total"]; $i++): @mysql_data_seek( $data["result"], $i );
                $ap        = @mysql_fetch_array( $data["result"] );
                $l         = $ap["id"];
                $AdminText .= "
                    <div class='question' id='question" . $l . "'>
                        <a href='javascript:void(0);' onclick='RemoveField(" . $l . ")'>Удалить</a>" . $C10 . "
	                    <textarea name='name[" . $l . "]' placeholder='Заголовок карточки'>" . $ap["name"] . "</textarea>" . $C10 . "
	                    <textarea name='text[" . $l . "]' placeholder='Текст карточки'>" . $ap["text"] . "</textarea>
                    </div>";
            endfor;
            $AdminText .= '</div>';
	...
	<div class='SecondMenu2'><a href='?cat=" . $alias . "_questions&id=" . $id . "'>Виджет: Ответы на вопросы</a></div>
