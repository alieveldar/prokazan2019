...
if (isset($_SESSION['Data']["sendbutton"])) {
	...
	$q="INSERT INTO `_widget_ask` (`link`,`pid`,`data`,`askname`,`asktext`)VALUES('nuriev','1','".time()."','".$name."','".$ptext."');"; DB($q); SD(); }
}
...
$text.='<div class="RoundText" id="Tgg"><form action="/modules/SubmitForm.php?bp='.$RealPage.'" enctype="multipart/form-data" method="post"><table><tr class="TRLine0"><td class="VarText">Имя или псевдоним</td><td class="LongInput"><input name="name" type="text" style="width:370px !important;"></td></tr><tr class="TRLine1"><td class="VarText" style="vertical-align:top; padding-top:10px;">Текст&nbsp;вопроса</td><td class="LongInput"><textarea name="text" style="height:50px; width:370px; !important;"></textarea></td></tr></table>'.$C10.'<div class="CenterText"><input type="submit" name="sendbutton" id="sendbutton" class="SaveButton" value="Задать вопрос"></div></form></div>'.$C30;

$data=DB("SELECT * FROM `_widget_ask` WHERE (`link`='nuriev' && `pid`='1') ORDER BY `id` DESC"); $text.="<table width='100%' cellpadding='0' cellspacing='0'>";
...
function askadmintext($ar) { global $C5, $RealPage;
	...
	$text.="<tr class='div".$ar["id"]." asktr'>";
		$text.="<td width='1%' class='uavatar'><img src='/userfiles/avatar/no_photo.jpg'></td>";
		$text.="<td width='99%' class='asktext'><textarea name='ask[".$ar["id"]."]' style='width:400px;' placeholder='Вопрос'>".($ar["asktext"])."</textarea></td>";
	$text.="</tr><tr class='div".$ar["id"]." anstr'><td width='1%' class='uavatar'>".$av."</td>";
		$text.="<td width='99%' class='asktext'><input name='face[".$ar["id"]."]' value='".$ar["ansname"]."' style='width:400px; margin-bottom:7px;' placeholder='Кто отвечает'><br>
		<textarea name='ans[".$ar["id"]."]' style='width:400px; height:100px; margin-bottom:7px;' placeholder='Ответ'>".$ar["anstext"]."</textarea>
