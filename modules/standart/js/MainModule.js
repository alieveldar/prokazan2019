...
$(function () {
    ...
    if( window.location.pathname.indexOf('/view/') && $('.ArticleContent').length ) {
        let timeout = null, cnt = 0;
        (function fixWidthTimeout() {
            if( 0 === $('#Banner-1-1').html().trim().length) {
                if( cnt < 50 ) {
                    timeout = setTimeout(fixWidthTimeout, 100);
                    cnt++;
                }
            } else {
                let banner = $('#Banner-1-1');
                let heightFix = +banner.offset().top + +banner.height();
                $('.ArticleContent').children().each(function (i, e) {
                    if ($(e).offset().top <= heightFix) {
                        $(e).css('maxWidth', '460px');
                    } else {
                        return false;
                    }
                });
    ...
    shuffle_avd('.news-avd');
    shuffle_avd('.news-avds')
});
...
function shuffle_avd(news_type) {
    let avd_news = $('.news-mid' + news_type), num = avd_news.length, list = [], looper = 0;
    for( let i = 0; i < num; i++ ) {
        if( looper > 1000000 ) {
            return;
        }
        let randomInt = Math.floor(Math.random() * num);
        if( list.indexOf( randomInt ) !== -1 && randomInt < num ) {
            i--;
            looper++;
            continue;
        }
    ...
    avd_news.each(function(i, news) {
        let insertion = $('<div></div>');
        insertion.attr('id', 'news_avd_temp_' + i);
        insertion.insertAfter(news);
    });
    list.forEach(function(news_id,i){
        if( i < (avd_news.length - 1) ) {
            avd_news.eq(news_id).insertAfter('#news_avd_temp_' + list[i+1]);
        } else {
            avd_news.eq(news_id).insertAfter('#news_avd_temp_' + list[0]);
        }
    });
    avd_news.each(function(i, news) {
        $('#news_avd_temp_' + i).remove();
    });
}