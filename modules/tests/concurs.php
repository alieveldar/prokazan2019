...
if ($start=="view") {
	...
	$data=DB("SELECT `$table`.`id`, `$table`.`domain`, `$table`.`name`, `$table`.`text`, `$table`.`comments`,
	`$table`.`votingend`, `$table`.`pic`, `$table`.`elemsstyle`, `$table`.`winnerscount`, `_pages`.`design`
	...
	$cap="Материал не найден"; $Page404=1; 
	}
}
...
if($start == 'list'){
	...
	} else { list($text, $cap)=GetLentaList();
	SetCache($file, $text, ""); }	
	$Page["Content"]=$text; 
	$cap=$node["name"];
}
