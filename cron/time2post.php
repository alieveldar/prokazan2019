...
$q=""; foreach($tables as $table) { $tmp=explode("_", $table); $link=$tmp[0]; $q.="(SELECT `$table`.`id`, `$table`.`name`, `$table`.`soctext` as `text`, `$table`.`data`, `$table`.`pic`, `_pages`.`link` FROM `$table` LEFT JOIN `_pages` ON `_pages`.`link`='$link' WHERE (`$table`.`stat`='1' && `$table`.`tavto`='1' && `$table`.`promo`!='1' && `$table`.`soctext`!='') GROUP BY 1) UNION ";}
$q=trim($q, "UNION ")." ORDER BY `data` DESC LIMIT 20"; //$rsstext.=$q; 
